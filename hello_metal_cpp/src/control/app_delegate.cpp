...
{
    CGRect frame = (CGRect){ {0.0, 0.0}, {600.0, 600.0} };
...
void AppDelegate::handleZoom(float delta)
{
    zoomFactor += delta;
    if (zoomFactor < 0.1f) zoomFactor = 0.1f; // Prevent too much zoom out
    viewDelegate->setZoomFactor(zoomFactor);
}

void AppDelegate::handlePan(float deltaX, float deltaY)
{
    panX += deltaX;
    panY += deltaY;
    viewDelegate->setPanOffset(panX, panY);
}
